...
public:
	...
	void send(int id, std::string message) {
		...
		if (ec) {
			std::cout << "∑¢ÀÕ¥ÌŒÛ‘≠“Ú£∫ " << ec.message() << std::endl;
			return;
		}
...
int main() {
	...
	std::string √‹¬Î;
	string ¿‡–Õ;
	std::string ¡¨Ω”Õ¯÷∑ = "ws://121.40.165.18:8800";
	std::cout << "«Î ‰»Î¡¨Ω”Õ¯÷∑£∫";
	getline(cin, ¡¨Ω”Õ¯÷∑);
	int id = endpoint.connect(¡¨Ω”Õ¯÷∑);
	if (id != -1) {

		std::cout << "> ƒ˙µƒ¡¨Ω”Œ™µ⁄  " << id << "∏ˆ" << std::endl;
	}
r:string d = "µ«»Î 0";
	std::stringstream ss(d);
	std::cout << endl << " ‰»Î’À∫≈:";
	std::getline(std::cin, ’À∫≈);
	std::cout << endl << " ‰»Î√‹¬Î:";
	std::getline(std::cin, √‹¬Î);
	string g = –¥»Î(√‹¬Î, ’À∫≈);
	std::string cmd;
